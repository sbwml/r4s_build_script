...
# MAIN
KERNEL_VERSION_MAIN=`curl -s https://us.cooluc.com/kernel/v6.x/sha256sums.asc | awk '{print $2}' | grep -E ^linux-6.2 | grep tar.xz | sed 's/linux-//g;s/.tar.xz//g' | tail -n 1`
...
[ "$1" = 2 ] && echo "LINUX_VERSION-6.2 = $TAG_MAIN
LINUX_KERNEL_HASH-$KERNEL_VERSION_MAIN = $KERNEL_HASH_MAIN" > $ROOT/kernel-6.2
